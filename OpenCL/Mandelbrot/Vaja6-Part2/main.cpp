...
int main(void){
	...
	size_t num_groups[2] = { ((height - 1) / local_item_size[0] + 1), ((width - 1) / local_item_size[0] + 1) };
	...
	ret = clEnqueueReadBuffer(command_queue, image_mem_obj, CL_TRUE, 0, height * width * 4 * sizeof(unsigned char), image, 0, NULL, NULL);
	// branje v pomnilnik iz naparave, 0 = offset
	// zadnji trije - dogodki, ki se morajo zgoditi prej

	//Shrani sliko:
	FIBITMAP* dst2 = FreeImage_ConvertFromRawBits(image, width, height, pitch, 32, FI_RGBA_RED_MASK, FI_RGBA_GREEN_MASK, FI_RGBA_BLUE_MASK, TRUE);
	FreeImage_Save(FIF_PNG, dst2, "mandelbrot.png", 0);
